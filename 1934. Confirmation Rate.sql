...
select s.user_id,
   round(sum(case when c.action = 'confirmed' then 1 else 0 end)*1.00/ iif(count(c.user_id)=0,1,count(c.user_id)),2) confirmation_rate
...
Select s.user_id, 
    Round(avg( (case when c.action = 'confirmed' then 1.0 else 0 end) ), 2)
